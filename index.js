...
console.log('Task 1 Part 1', artists[0].name)


//(2) Bio of the third artist (2nd index) in the array 

console.log('Task 1 Part 2', artists[2].bio)
...
function lotsOfArt(array) {
  const paintings = array.filter(paintingResults => {
    return paintingResults.paintings > 100})
    const paintingNames = paintings.map(filteredNames =>
    filteredNames.name)
...
function artistInfo(array, name){
  const filteredInfo = array.filter(filteredInfo => {
    return filteredInfo.name === name})
    return filteredInfo[0].bio;
}
...
function artistByCountry(array, nationality){
  const artistsNationality = artists.filter(filteredNationality => {return filteredNationality.nationality === nationality});
  return artistsNationality.map(parameter => parameter.name);
}
